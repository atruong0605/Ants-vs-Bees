...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if len(place.bees) > 0  and self.lower_bound <= place_holder <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        reflective = amount + self.damage if self.health <= amount else amount
        for i in self.place.bees[:]:
            i.reduce_health(reflective)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_present == False:
            gamestate.queen_present = True
    ...
    def action(self, gamestate):
        ...
        damage_place = self.place.exit
        while True:
            if damage_place == None:
                return
            elif damage_place.ant != None:
                if damage_place.ant.is_container and damage_place.ant.ant_contained:
                    damage_place.ant.ant_contained.double()
                    damage_place.ant.double()
                    damage_place = damage_place.exit
                else:
                    damage_place.ant.double()
                    damage_place = damage_place.exit
            else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.turns_slowed > 0:
            self.turns_slowed = self.turns_slowed - 1
        ...
        if self.turns_scared:
            if self.place.ant:
                ...
                return
            elif self.place.exit.is_hive == False:
                destination = self.place.entrance
                self.turns_scared = self.turns_scared - 1
            elif self.place.exit.is_hive:
                self.turns_scared = self.turns_scared - 1
                return
